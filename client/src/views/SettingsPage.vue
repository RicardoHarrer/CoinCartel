        ...
        <q-form @submit="savePreferences" class="q-gutter-md">
          ...
          >
            <template v-slot:no-option>
              <q-item>
                <q-item-section class="text-grey"> No currencies available </q-item-section>
              </q-item>
            </template>
          </q-select>
          ...
          <div class="row justify-end q-mt-md">
            <q-btn label="Save Settings" type="submit" color="primary" :loading="loading" />
          </div>
        </q-form>
      </q-card-section>
    </q-card>
...
export default defineComponent({
  ...
  setup() {
    ...
    const convertedBudget = computed(() => {
      ...
      return (form.value.saldo * exchangeRates.value[form.value.preferred_currency]).toFixed(2);
    });
